...
class DataHandler:
    ...
    def check_if_bar_can_be_generated(self):
        ...
        if self.volumen_acumulado >= threshold_bar:
    ...
    def crear_features_y_predecir(self):
        ...
        # Si no se han generado las barras suficientes, retornar
        if len(self.barras < self.threshold_bar): 
            return
        
        # Aquí puedes calcular los features basados en las barras OHLC
        df_barras = pd.DataFrame(self.barras[-self.threshold_bar:])  # Seleccionar las últimas "threshold_features" barras
        features = calcular_features(df_barras, self.threshold_bar)
        
        # Aquí haces el escalado y predicciones con el modelo cargado
        print(f"Features creados para {self.ticker}: {features}")

        features_scaled = self.scaler.transform([features])

        # Hacer predicción con el modelo de árbol de decisión
